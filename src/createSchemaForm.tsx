...
>(
  ...
  options?: {
     ...
     * components with `@cxnpl/react-ts-form` or modify its behavior. The values of the object are the names of the props to forward the corresponding
     * data to.
     * @default {
     *  name: "name",
     *  control: "control",
        ...
        if (isAnyZodObject(type)) {
          ...
          return Object.entries(shape).reduce((accum, [subKey, subType]) => {
            accum[subKey] = renderComponentForSchemaDeep(
              subType,
              props && props[key] ? (props[key] as any) : undefined,
              subKey,
              `${prefixedKey}.${subKey}`,
